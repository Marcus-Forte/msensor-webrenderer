...
function animate() {
  ...
  // Get forward and right vectors relative to camera orientation
  ...
  // Continuous movement logic (relative to camera)
  if (keysPressed['w'] || keysPressed['arrowup']) camera.position.add(forward.clone().multiplyScalar(moveSpeed));
  if (keysPressed['s'] || keysPressed['arrowdown']) camera.position.add(forward.clone().multiplyScalar(-moveSpeed));
  if (keysPressed['a'] || keysPressed['arrowleft']) camera.position.add(right.clone().multiplyScalar(-moveSpeed));
  if (keysPressed['d'] || keysPressed['arrowright']) camera.position.add(right.clone().multiplyScalar(moveSpeed));
  if (keysPressed['q']) camera.position.y += moveSpeed;
  if (keysPressed['e']) camera.position.y -= moveSpeed;


  // Camera orbits around the target
  // camera.position.x = target.x + Math.sin(yaw) * Math.cos(pitch) * orbitRadius;
  // camera.position.y = target.y + Math.sin(pitch) * orbitRadius;
  // camera.position.z = target.z + Math.cos(yaw) * Math.cos(pitch) * orbitRadius;
...
client
  ...
  .on('data', (response: PointCloud3) => {
    ...
    points.forEach((point, i) => {
      ...
      colors[i * 3] = point.hasR() ? point.getR()! / 255 : 1.0;
      colors[i * 3 + 1] = point.hasG() ? point.getG()! / 255 : 1.0;
      colors[i * 3 + 2] = point.hasB() ? point.getB()! / 255 : 1.0;
